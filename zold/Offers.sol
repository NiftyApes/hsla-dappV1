  ...
  IOffers
{
  ...
  function updateLendingContractAddress(address newLendingContractAddress)
    external
    onlyOwner
  {
  ...
  /// @inheritdoc IOffers
  function createOffer(Offer memory offer)
    external
    whenNotPaused
    returns (bytes32 offerHash)
  {
    address cAsset = ILiquidity(liquidityContractAddress).getCAsset(
      offer.asset
    );
    ...
    if (offer.lenderOffer) {
      uint256 offerTokens = ILiquidity(liquidityContractAddress)
        .assetAmountToCAssetAmount(offer.asset, offer.amount);
  ...
  function _requireOfferCreatorOrLendingContract(
    address signer,
    address expected
  ) internal view {
  ...
  function _requireSigLendingContract() internal view {
    ...
    console.log('sigLendingContractAddress', sigLendingContractAddress);
  ...
  function _requireCAssetBalance(
    ...
    require(
      ILiquidity(liquidityContractAddress).getCAssetBalance(account, cAsset) >=
        amount,
      '00034'
    );
  }
