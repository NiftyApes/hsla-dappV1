...
export const DepositLiquidity: React.FC = () => {
    ...
    const {depositETHLiquidity, depositStatus, txReceipt} = useDepositEthLiquidity();
    ...
    const isDepositBtnDisabled = depositStatus !== 'READY' || isInputBlank || !isInputConvertibleToNumber || doesInputExceedsMax;
    ...
    useEffect(() => {
        ...
        if (depositStatus === 'SUCCESS' && txReceipt) {
            toast({
                render: (props) => <ToastSuccessCard title="Liquidity Deposited" txn={txReceipt} {...props} />,
                position: 'top-right',
                duration: 9000,
                isClosable: true,
            });
